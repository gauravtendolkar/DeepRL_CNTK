...
def run(render=False):
    ...
    while True:
        ...
        if is_done:
            agent.learn()
            agent.actor_policy.probabilities.save("pre_trained/a2c_pong/v2/Pong-v0_reward_{}.actor.model".format(cumulative_reward))
            agent.critic_policy.value.save("pre_trained/a2c_pong/v2/Pong-v0_reward_{}.critic.model".format(cumulative_reward))
...
while ep < NUM_EPISODES:
    ...
    with open("pre_trained/a2c_pong/v2/episode_rewards.txt", "w") as f:
        f.write(str(episode_rewards))
    print("Episode {} Terminated with reward {}, steps: {}, lr: {}..".format(ep, episode_reward, agent.steps, agent.actor_policy.trainer.parameter_learners[0].learning_rate()))
    ep += 1
    if ep > 101:
        avg_reward = np.mean(episode_rewards[-100:])
        print("Episode {}: Average Reward in past 100 eisodes {}, Epsilon: {}, Steps: {}, LR: {}".format(ep, np.mean(episode_rewards[-100:]),
                                                                                                agent.epsilon,
                                                                                                agent.steps, agent.actor_policy.trainer.parameter_learners[0].learning_rate()))
        if avg_reward > 20:
            break

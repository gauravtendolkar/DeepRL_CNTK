...
class ActorCriticCNNPolicy:
    ...
    def _build_network(self, pretrained_policy):
        ...
        if pretrained_policy is None:
            ...
            self.output = C.combine([self.probabilities, self.value])
        else:
            [self.probabilities, self.value] = list(C.Function.load(pretrained_policy)(self.image_frame))
        selected_action_probablity = C.ops.times_transpose(self.probabilities, one_hot_action)
        self.log_probability = C.ops.log(selected_action_probablity)
        self.actor_loss = -self.td_error * self.log_probability

        self.critic_loss = C.squared_error(self.target_current_state_value, self.value)
    ...
    def optimise(self, image_frame, td_error, action_index, target_current_state_value):
        self.trainer.train_minibatch({self.image_frame: image_frame, self.td_error: td_error,
                                      self.action_index: action_index, self.target_current_state_value: target_current_state_value})

    def predict(self, image_frame):
        return self.probabilities.eval({self.image_frame: image_frame})

    def value(self, image_frame):
        return self.value.eval({self.image_frame: image_frame})
